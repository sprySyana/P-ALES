...
def extractPairs(folderLexicon,folderPhonemes,folderMain):
    ...
    #   Dictionary to stock each phonem paire(Key) with its frequency(Value)
    dicPairs = {}
    # Liste qui stocke les phonèmes rencontrés pour chaque langue
    #   List of phonems for the currently treated langue
    listP = []
    # Compteur du nombre de paires recensées
    #   Counter of (different) pairs met 
    # ~ nb_occ = 0
    # Création du dossier de sortie s'il n'existe pas
    #   Create folder if not existing or path not specified
    if not os.path.isdir(folderPhonemes):
        os.mkdir(folderPhonemes)
    try:
        # On obtient la liste des fichiers du répertoire
        #   Retrieve list of folder in directory
    ...
    #   Open lexicon files one by one
    for f in files:
        try:
            lexPho = open(os.path.join(folderLexicon,f),"r",encoding="utf8")
            # On récupère la langue dans le nom des fichiers
            #   Retrieve langue in file name
            m = re.search(r"(.*)([_].*)",f)
            language = m.group(1)
            # Parcours du lexique phonétisé
            #   Scan phonetised lexicon
            for line in lexPho:
                phono = line.split(" ")
                # ~ print (phono)
        ...
        #   Write the phonem list in the appropriate file
        try: 
            # Création du fichier contenant tous les phonèmes
            #   Creation of file containing all phonems
