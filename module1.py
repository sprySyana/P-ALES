		...
		try:
			...
			for line in lexPho:
				...
				for i in range(0,len(phono)-1):
					...
					if (phono[i] == "ɔ" or phono[i] == "ɑ" or phono[i] == "ɛ") and (phono[i+1] == "̃"):
						courant = phono[i] + phono[i+1]
						i += 1
					# Cas normal
					elif phono[i] != "̃":
						courant = phono[i]
					# On traite ensuite l'élément suivant (2ème élément de la paire)
					# On traite d'abord le cas des voyelles nasales (2 caractères en API)
					if (phono[i+1] == "ɔ" or phono[i+1] == "ɑ" or phono[i+1] == "ɛ") and (phono[i+2] == "̃"):
